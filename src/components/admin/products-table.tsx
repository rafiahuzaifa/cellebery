                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button
                      ...
                      disabled={isPending}
                      onClick={() => runAction(() => setAdminProductStatusAction(product.id, product.status === "active" ? "draft" : "active"))}
                      className="grid size-8 place-items-center rounded-lg text-white/50 transition hover:bg-white/5 hover:text-white disabled:opacity-40"
                    >
                    ...
                    <button aria-label="Duplicate" disabled={isPending} onClick={() => runAction(() => duplicateAdminProductAction(product.id))} className="grid size-8 place-items-center rounded-lg text-white/50 transition hover:bg-white/5 hover:text-white disabled:opacity-40"><Copy size={14} /></button>
                    <button aria-label="Delete" disabled={isPending} onClick={() => { if (confirm(isArabic ? "حذف هذا المنتج؟" : "Delete this product?")) runAction(() => deleteAdminProductAction(product.id)); }} className="grid size-8 place-items-center rounded-lg text-white/50 transition hover:bg-red-500/10 hover:text-red-300 disabled:opacity-40"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
