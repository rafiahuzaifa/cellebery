  ...
  return (
    <main className="overflow-hidden bg-[#080a0c] text-[#f3f5f5]">
      <section className="relative min-h-[720px] h-screen max-h-[980px] overflow-hidden border-b border-white/10">
        ...
        <div id="top" className="relative z-10 mx-auto flex h-[calc(100%-100px)] max-w-[1440px] items-center px-6 py-16 lg:px-12">
          <div className="grid w-full items-center gap-8 lg:grid-cols-[minmax(420px,0.95fr)_minmax(420px,1.05fr)]">
            <div className="relative min-h-[420px] lg:min-h-[560px]">
              <MediaVideo src="/Person_holding_Bluetooth_speaker_202609080042.mp4" poster="/ChatGPT%20Image%20Aug%2027,%202026,%2005_23_32%20PM.png" className="absolute inset-0 h-full w-full rounded-[2rem] border border-white/15 opacity-85 shadow-[0_0_60px_rgba(159,246,237,0.14)]" label="Toggle sound" showSoundButton={false} />
              <div className="absolute inset-0 rounded-[2rem] border border-[#9ff6ed]/30 bg-gradient-to-t from-[#080a0c]/40 via-transparent to-[#9ff6ed]/10" />
            </div>

            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-2xl lg:pl-8">
              ...
              <h1 className="display-font max-w-xl text-7xl font-semibold uppercase leading-[0.82] sm:text-8xl lg:text-[8rem]">
                {isArabic ? <>
      ...
      <section id="shop" className="mx-auto max-w-[1440px] px-6 py-24 lg:px-12 lg:py-36">
        <div className="mb-12 flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="mb-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#9ff6ed]">{copy.collection}</p>
            <h2 className="display-font text-5xl font-semibold uppercase sm:text-7xl">{isArabic ? <>اختر<br />صوتك.</> : <>Choose<br />your sound.</>}</h2>
          </div>
          <Link href="/shop" className="hidden items-center gap-2 text-[10px] uppercase tracking-[0.17em] text-white/60 transition hover:text-[#9ff6ed] sm:flex">{isArabic ? "عرض كل المنتجات" : "View all products"} <ArrowUpRight size={15} /></Link>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {categories.map((category, index) => (
            <motion.a whileHover={{ y: -5 }} transition={{ duration: 0.25 }} href={`/shop?category=${category.label.toLowerCase()}`} key={category.label} className="group relative aspect-[0.82] overflow-hidden bg-[#151a1c] p-6 sm:p-8">
              ...
              <div className="absolute inset-0 border border-white/10 transition group-hover:border-[#9ff6ed]/60" />
              <div className="relative flex h-full flex-col justify-between">
                <span className="flex size-9 items-center justify-center rounded-full border border-white/30 text-xs text-white/60">0{index + 1}</span>
                <div>
                  ...
                  <span className="mt-7 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#9ff6ed] opacity-0 transition group-hover:opacity-100">Explore <ArrowUpRight size={14} /></span>
                </div>
              </div>
            </motion.a>
          ))}
      ...
      <section id="featured" className="relative border-y border-white/10 bg-[#101416] px-6 py-24 lg:px-12 lg:py-32">
        <div className="mx-auto grid max-w-[1440px] items-center gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24">
          <div>
            <p className="mb-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#9ff6ed]">{copy.featured}</p>
            ...
            <div className="mt-10 flex items-center gap-5">
              <Link href="/shop/x7-pro" className="inline-flex items-center gap-6 bg-white px-5 py-4 text-[10px] font-bold uppercase tracking-[0.18em] text-[#080a0c] transition hover:bg-[#9ff6ed]">{copy.discoverX7} <ArrowUpRight size={15} /></Link>
              <span className="text-sm text-white/45">SAR 499</span>
            </div>
          </div>
          <div className="relative aspect-[1.1] overflow-hidden bg-[#171d1e]">
            <div className="full-media absolute inset-0 opacity-80" style={{ backgroundImage: "linear-gradient(90deg, rgba(16,20,22,.1), rgba(16,20,22,.3)), url('/ChatGPT%20Image%20Aug%2027,%202026,%2005_22_17%20PM.png')" }} />
            <div className="absolute inset-0 bg-gradient-to-t from-[#080a0c]/70 via-transparent to-transparent" />
            <div className="absolute bottom-6 left-6 flex gap-2">{stats.map(([value, label]) => <div key={value} className="border border-white/20 bg-[#080a0c]/75 px-3 py-3 backdrop-blur-md"><p className="text-sm font-semibold text-[#9ff6ed]">{value}</p><p className="mt-1 text-[8px] uppercase tracking-[0.12em] text-white/55">{label}</p></div>)}</div>
