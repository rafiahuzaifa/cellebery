...
export default function ProductPage() {
  ...
  return (
    <main className="min-h-screen bg-[#080a0c] text-[#f3f5f5]">
      ...
      <section className="mx-auto grid max-w-[1440px] gap-12 px-6 py-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:gap-24 lg:px-12 lg:py-20"><div className="relative aspect-square overflow-hidden border border-white/10 bg-[#151a1c] shadow-2xl shadow-cyan-950/20"><div className="absolute inset-0 bg-cover bg-center opacity-90 transition duration-700 hover:scale-105" style={{ backgroundImage: `linear-gradient(135deg, rgba(8,10,12,.02), rgba(8,10,12,.48)), url(${product.image})` }} /><div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#080a0c]/80 to-transparent" /><span className="absolute bottom-6 left-6 border border-[#22d3ee]/40 bg-[#080a0c]/70 px-3 py-2 text-[9px] uppercase tracking-[0.16em] text-[#22d3ee] backdrop-blur-sm">{labels.designed}</span></div><div><p className="mb-5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#22d3ee]">{product.category}</p><h1 className="display-font text-7xl font-semibold uppercase leading-[0.85] sm:text-8xl">{product.name}</h1><p className="mt-8 max-w-md text-base leading-8 text-white/55">{isArabic ? (product.name === "X7 Pro" ? "صوت غامر، وإلغاء ضوضاء متكيف، وراحة طوال اليوم مصممة لحركتك." : "حجم صغير، صوت كبير. تجربة استماع بحجم الجيب مع وضوح يرافقك.") : product.description}</p><div className="mt-8 flex items-center gap-5"><span className="text-2xl">SAR {product.price}</span><span className="text-xs text-white/40">{labels.freeDelivery}</span></div><button onClick={() => { addItem({ id: slug, name: product.name, price: product.price }); setAdded(true); }} className="mt-9 flex w-full items-center justify-between bg-[#22d3ee] px-5 py-4 text-[10px] font-bold uppercase tracking-[0.17em] text-[#080a0c] transition hover:bg-white sm:w-80">{added ? labels.added : labels.add} <ArrowUpRight size={16} /></button><div className="mt-10 grid gap-4 border-t border-white/10 pt-6 sm:grid-cols-3"><div className="flex gap-3 text-[10px] uppercase leading-4 tracking-[0.1em] text-white/55"><Truck size={16} className="shrink-0 text-[#22d3ee]" /> {labels.delivery}</div><div className="flex gap-3 text-[10px] uppercase leading-4 tracking-[0.1em] text-white/55"><ShieldCheck size={16} className="shrink-0 text-[#22d3ee]" /> {labels.warranty}</div><div className="flex gap-3 text-[10px] uppercase leading-4 tracking-[0.1em] text-white/55"><Check size={16} className="shrink-0 text-[#22d3ee]" /> {labels.returns}</div></div></div></section>
      <section className="border-y border-white/10 bg-[#0d1113] px-6 py-16 lg:px-12"><div className="mx-auto max-w-[1440px]"><p className="mb-8 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#22d3ee]">{labels.engineered}</p><div className="grid gap-0 border-l border-white/10 sm:grid-cols-2 lg:grid-cols-4">{features.map((feature) => <div key={feature} className="border-b border-r border-white/10 px-5 py-6 text-sm text-white/70 lg:border-b-0"><div className="mb-8 size-2 bg-[#22d3ee]" />{feature}</div>)}</div></div></section>
